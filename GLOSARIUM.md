...
| `sp_buat_akun_kasir` | `POST /api/v1/akun/kasir` | Verifikasi manajer aktif, lalu INSERT pengguna baru dengan `peran = 'kasir'`. Password sudah di-hash di Node.js (controller `akun.controller.js`) sebelum dikirim ke procedure ini. |
| `sp_toggle_status` | `PATCH /api/v1/sistem/status` | Verifikasi manajer aktif, lalu UPDATE kolom `is_active` pada entitas target. Parameter `entitas` harus salah satu dari: `'akun'`, `'barang'`, atau `'kategori'`. |
